...
Incluye tablas para usuarios, proyectos, paquetes, catálogo de actividades, actividades y planes de trabajo.
...
## Migrar tablas desde el código
En este MVP no hay migraciones automáticas. Por ahora debes ejecutar el script manual:
```sql
sql/timesheet_schema.sql
```

Cuando integremos Prisma, el flujo será:
```bash
...
2) En la parte superior verás la barra con tres opciones:
   - **Actividades**: formulario para registrar horas y la lista de actividades.
   - **Reportes**: resumen por proyecto/usuario y exportación JSON.
   - **Plan de trabajo**: el supervisor puede cargar estimaciones del equipo.

## Cómo cargar horas (Actividades)
1) Ve a **Actividades** en el menú.
2) Completa:
   - Usuario
   - Proyecto
...
4) Las actividades aparecerán en la lista derecha y se pueden editar o eliminar.

## Cargar un Excel (flujo sugerido)
Este MVP no importa Excel aún, pero el flujo recomendado será:
1) Exportar tu Excel a **CSV**.
2) Crear un endpoint `/api/import` que lea el archivo CSV.
3) Convertir cada fila en una actividad y guardarla en la BD.

Cuando implementemos el backend puedo agregarte un importador con `exceljs`.
